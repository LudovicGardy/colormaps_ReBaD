...
def create_cmap(cmatrix_dict: dict, combine_two_cmaps: bool = True) -> tuple:
    """
    ...
    if combine_two_cmaps:
        ...
        color_matrix[cmatrix_2D_length:, 3] = np.repeat(
            1, len(cmatrix_dict["b"])
        )  # alpha channel
    else:
...
def get_all_cmaps(HaReBaD_color_matrix_upper: np.ndarray) -> object:
    """
